...
int RunGame()
{
	...
	SoundBuffer buffer;
	buffer.loadFromFile("files/Jump.ogg");
	Sound sound(buffer);
	...
	Level lvl;
	lvl.LoadFromFile("files/Level.tmx");
	...
	while (window.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Up)) {
			if (Mario.STATE == 0 || Mario.STATE == 1) {
				sound.play();
			}
			Mario.key["Up"] = true;
		}
		...
		for (it = entities.begin(); it != entities.end(); it++)
		{
			...
			if ((*it)->Name == "Enemy")
			{
				...
				if (Mario.getRect().intersects(enemy->getRect()))
					if (Mario.dy > 0) { enemy->dx = 0; Mario.dy = -0.2; enemy->Health = 0; --enemy_count; }
					else if (!Mario.hit) {
			...
			if (enemy_count == 0) { return 0; window.close(); }
